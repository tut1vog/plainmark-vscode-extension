...
- **MRS-R-4** — For a non-empty selection, the construct MUST reveal unless the selection strictly extends past it on BOTH sides (`r.from < node_from && r.to > node_to`); a selection that touches, partially overlaps, sits strictly inside, or is boundary-equal MUST reveal. A selection edge at a document edge counts as extending past: a construct that opens the document has no byte before it, and one that closes it has none after, so a selection reaching the document start or end on that side covers the construct there — select-all therefore never reveals, whatever the construct's position (IMG-I-1, MMD-I-1).
  _Example:_ select an entire line containing `x **bold** y` → markers stay hidden (strict cover); select `**bold**` exactly → revealed; in `**bold** y`, select from the document start to the line end → hidden; Ctrl+A on a document that is one image or diagram → the widget stays rendered.
...
- **MRS-B-1** — When the caret sits at the end of a canonical block marker (`>`, `-`/`*`/`+`, or `n.`/`n)`, each followed by one space, possibly nested) AND there is EXTRA whitespace immediately after that marker before content, Backspace MUST delete exactly one character at the caret and MUST NOT delete the marker. A bullet's GFM task checkbox (`[ ]`/`[x]`/`[X]`, 1–4 spaces after the bullet) counts as part of the marker — mirroring `deleteMarkupBackward`'s own bullet-context pattern, which treats the checkbox as markup and would otherwise delete bullet + checkbox together.
  _Example:_ `>  hello` with caret at column 2 → Backspace → `> hello` (caret at column 1), marker preserved. `- [ ]  hello` with caret at column 6 → Backspace → `- [ ] hello` (caret at column 5), bullet and checkbox preserved.
