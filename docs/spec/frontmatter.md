...
- **FM-E-9** — A frontmatter block always begins at document offset 0. Under MRS-R-4 the document start counts as extending past the block on that side, so a select-all (or any selection from offset 0 running past the closing `---`) covers the block and keeps the `---` fences HIDDEN — the same result the gesture has over a mid-document fenced-code block; a selection from offset 0 that ends inside the block or exactly at its end still reveals them.
  _Example:_ select-all over `---\nfoo: bar\n---\nProse` → the `---` fences stay hidden, like the ` ``` ` of a mid-document code block; select `---\nfoo: bar\n---` exactly → both fences reveal.
