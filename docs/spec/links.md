...
- **LINK-R-2** — The bracketed link text (between the opening `[` and the closing `]`, i.e. `[open.to, close_bracket.from)`) MUST receive a `Decoration.mark` with class `plainmark-link` carrying a `data-plainmark-href` attribute whose value is the effective destination of the `URL` child per LINK-R-3 (empty string when there is no `URL` child).
  _Example:_ `[t](u)` → mark over `t` (offsets 5–6) with `data-plainmark-href="u"`.

- **LINK-R-3** — The href attribute MUST carry the effective destination: the verbatim `URL`-child slice for a bare destination, and the slice with the surrounding `<`/`>` delimiters stripped for a CommonMark angle-bracket destination — the lezer `URL` node includes those delimiter bytes. Absolute and relative targets are otherwise captured as-is — no percent-decoding, no normalization — with resolution deferred to the host.
  _Example:_ `[Plainmark](https://example.com)` → `data-plainmark-href="https://example.com"`; `[docs](./guide)` → `data-plainmark-href="./guide"`; `[book](<a b.pdf>)` → `data-plainmark-href="a b.pdf"`.
