...
- **CBLK-R-18** — Leading whitespace up to the opening fence's indent (the character count before the opening `CodeMark` on its line) MUST be hidden on every line of the block via a widgetless `Decoration.replace({})` (the LIST-R-5 / BQ-R-14 leading-whitespace idiom) — CommonMark strips that indent from the block's content, so displayed code sits flush at the box's padding edge. NOT a zero-font mark: a tab's advance runs to the block container's next tab stop (computed from the content edge in the block's font), which a zero font on the span does not shrink, so a tab-indented block would render its code one tab stop in. A line with fewer leading whitespace characters hides only what is there; whitespace past the fence indent stays visible (intentional code indentation); a blank or flush-left line takes no decoration (a zero-length range is illegal in CM6). The hide is emitted in BOTH caret states and never reveals: a source-true indent on the caret line would shift the line on caret enter/leave (the LIST-I-3 defect class); caret positions inside the hidden run collapse to one x, the accepted cost the list constructs already carry.
