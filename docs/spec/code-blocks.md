...
- **CBLK-R-16** `[smoke]` — The `codeLanguages` matcher (`match_code_language`) MUST resolve additional commonly-typed fence tags onto grammars `@codemirror/language-data` already bundles, via wrapper `LanguageDescription`s that delegate `load()` to the base entry — same `LanguageSupport` instance, no new grammar imports (e.g. `asm`/`assembly`/`nasm`/`x86asm` → Gas, `wasm` → WebAssembly, `matlab` → Octave, `py` → Python). The alias layer participates in EXACT name/alias matching only, then falls back to the stock registry with its default fuzzy matching — an alias must never fuzzy-capture an unrelated tag (```armasm is a different instruction set and stays unresolved). An alias lands only when documented by highlight.js, Linguist, or Typora, or when the tag names a dedicated bundled grammar; cross-language approximations (`asm` → GAS AT&T-syntax grammar, `tsql` → MS SQL, `gradle` → Groovy DSL) follow shipped precedent in those sources.
  _Example:_ ` ```asm\nmovl $1, %eax  # exit\n``` ` colors the comment via the Gas grammar; ` ```armasm ` renders un-tokenized monospace.

- **CBLK-R-17** `[smoke]` — A `FencedCode` nested in list items (≥1 enclosing `ListItem`, outside any `Blockquote`) MUST render at the list content column: every line of the block carries class `plainmark-fenced-code-nested` plus `--plainmark-list-depth: <n>` (n = the count of enclosing `ListItem`s, uncapped), and CSS shifts the tinted box (background inset and position) and the line's `padding-left` right by `n × --plainmark-list-indent` beyond the CBLK-R-4 geometry, landing the box's left edge on the depth-grid column the item's wrapped rows hang at (LIST-E-4). The gapped opening line keeps the offset (a triple-class rule outranks the CBLK-R-5 gapped-header background rule). The nested class and depth style are emitted in BOTH caret states.
  _Example:_ `- item\n  ```js\n  const x = 1;\n  ``` ` → the code box starts under `item`, not at the editor margin.

- **CBLK-R-18** — Leading whitespace up to the opening fence's indent (the column count before the opening `CodeMark` on its line, a tab advancing to the next multiple of 4 as in CommonMark) MUST be hidden on every line of the block via a widgetless `Decoration.replace({})` (the LIST-R-5 / BQ-R-14 leading-whitespace idiom) — CommonMark strips that indent from the block's content, so displayed code sits flush at the box's padding edge. NOT a zero-font mark: a tab's advance runs to the block container's next tab stop (computed from the content edge in the block's font), which a zero font on the span does not shrink, so a tab-indented block would render its code one tab stop in. A line with fewer leading whitespace columns hides only what is there; a leading tab is hidden only when its full advance fits within the fence indent (a tab that would overshoot stays visible rather than swallowing more indent than the fence strips); whitespace past the fence indent stays visible (intentional code indentation); a blank or flush-left line takes no decoration (a zero-length range is illegal in CM6). The hide is emitted in BOTH caret states and never reveals: a source-true indent on the caret line would shift the line on caret enter/leave (the LIST-I-3 defect class); caret positions inside the hidden run collapse to one x, the accepted cost the list constructs already carry.
  _Example:_ `1. a\n   ```py\n   x = 1\n       y = 2\n   ``` ` → `x = 1` renders flush inside the box, `y = 2` keeps four visible spaces; caret enter/leave shifts nothing. `  ```\n\tx\n  ``` ` (2-space fence) → the tab stays visible, since its 4-column advance exceeds the fence indent.
...
- **CBLK-E-8** `[accepted]` — Inside a `Blockquote` (callouts included) the CBLK-R-17/CBLK-R-18 geometry MUST NOT apply: quoted fence lines keep the pre-existing rendering — no nested class, no depth style, no indent hide — with the in-flow `> ` prefix and any nesting spaces after it visible. The CBLK-I-2 fence hide MUST start at the fence run, not the line start, so the quote marker keeps its glyph box (BQ-R-11) on fence rows exactly as on body rows. Content-column alignment for quote-nested fenced code is deferred: the quote's net-to-zero hanging indent and per-depth line padding (BQ-R-12) own quoted geometry, and composing the two models is its own decision.
  _Example:_ `> - a\n>   ```js ` → block chrome as before this rule; the two nesting spaces after `> ` stay in flow.
