...
- **CBLK-R-18** — Leading whitespace up to the opening fence's indent (the character count before the opening `CodeMark` on its line) MUST be display-hidden on every line of the block via a zero-font `plainmark-fenced-code-indent` `Decoration.mark` — CommonMark strips that indent from the block's content, so displayed code sits flush at the box's padding edge. A line with fewer leading whitespace characters hides only what is there; whitespace past the fence indent stays visible (intentional code indentation); a blank or flush-left line takes no mark (a zero-length mark is illegal in CM6). The hide is emitted in BOTH caret states and never reveals: a source-true indent on the caret line would shift the line on caret enter/leave (the LIST-I-3 defect class); caret positions inside the hidden run collapse to one x, the accepted cost the list constructs already carry.
