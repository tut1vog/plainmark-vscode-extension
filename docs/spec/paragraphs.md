...
- **PARA-R-3** — A blank line MUST act as a paragraph separator in source semantics (plain CommonMark: consecutive non-blank lines are one paragraph, a blank line splits two); visually, blank lines render as normal-height caret-placeable rows. The first blank line of a run carries the PARA-R-7 gap; every later line of the run sits bare below a blank (PARA-R-13).
  _Example:_ `a\nb` → one CommonMark paragraph across two source lines; `a\n\nb` → two paragraphs with a visible blank row between them, the seam's single gap on the blank row.
...
- **PARA-R-8** — List-line eligibility MUST be seam-local. In-list lines on which no `ListItem` starts — item continuations (lazy `- a\nb`, indented `- a\n  b`, a loose item's later paragraphs) and blank lines between items — are gap-eligible, so a hard `\n` after a bullet reads as a paragraph break and a blank run between items renders as the same run would outside the list. A line a `ListItem` starts on is gap-eligible UNLESS a `ListItem` also starts on the line directly above (the tight seam, owned by the adjacent-sibling item spacing) — first-of-list and interior markers alike, independent of the marker's role in the parse. Extends PARA-E-5's recorded divergence: GitHub renders a marker directly under a continuation line tight; here it carries the gap.
  _Example:_ `para\n- x\n- y` → gap above `- x`; `- y` gets only the tight item spacing. `- a\n\n- b` → gap on the blank line only; `- b` sits bare below it (PARA-R-13).
...
- **PARA-R-12** — Gap eligibility MUST be invariant under edits to other lines: a line's gap may change only when that line or the line directly above it is edited. Marker-typing transitions (`para\n-` setext state, `para\n* ` paragraph state, `para\n* x` list state) keep the gap; converting a paragraph line into a fence, HR, heading, or quote first line keeps it (the construct start is itself eligible); deepening an interior quote line with another `>` keeps it; typing or deleting between two list items — splitting the spanning list or merging blocks into it via lazy continuation — moves no flag beyond the line directly below the edit. The permitted moves are the semantic ones: completing a bullet on a line hands its gap to item spacing in that keystroke; a marker line moves only when the line above becomes or ceases to be a marker line (a tight seam forming or dissolving); and the line directly below an edit flips when that edit creates or dissolves a blank seam (PARA-R-13).
  _Example:_ `- abc\n\n- def` → typing `x` on the blank line re-gaps `- def` (its blank seam dissolved); deleting `x` back out bares it again; no other line's gap moves in either direction.

- **PARA-R-13** `[smoke]` — A blank line owns its seam's single gap: a line directly below a blank line — prose, blank, marker, or construct start — MUST NOT carry the gap, so a blank-separated seam costs one gap (not two) and a blank run compacts after its first line. A line is blank when it contains only whitespace and `>` quote marks (the lexical quote prefix, BQ-R-12's scan), so quote-interior separator lines count. The three block widgets mirror the rule on their gap flag (TBL-R-10, MATH-R-7, IMG-R-11), so a widget and its revealed source agree on the seam and caret entry/exit never shifts the layout. A construct-start line below a blank reverts to its non-gapped spacing — its doc-top state (e.g. a heading's scaled breathing, HEAD-R-9). A doc-top blank line yields a bare line 2 (the doc-top region has no seam).
  _Example:_ `a\n\nb` → one gap, on the blank row; `b` is bare. `a\n\n\n\nb` → gap on the first blank only. `> a\n>\n> b` → gap on the `>` line only.
