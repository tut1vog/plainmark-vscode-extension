...
- **PARA-R-7** — Every gap-eligible line after the first document line MUST carry paragraph-gap padding-top (`--plainmark-paragraph-gap`, default `0.75em`) — padding, never margin (CM6 height-map rule). Gap-eligible: prose lines, blank lines, setext-heading lines (unstyled in Plainmark), the first line of a top-level list, in-list lines on which no `ListItem` starts — item continuation lines continuing an item begun on an earlier line (lazy `- a\nb`, indented `- a\n  b`, and a loose item's later paragraphs), so a hard `\n` after a bullet reads as a paragraph break, not a soft wrap, AND every line a `ListItem` starts on EXCEPT at a tight seam — marker-line spacing is seam-LOCAL: a marker line directly under another marker line is tight (the adjacent-sibling item spacing owns that seam), and under anything else (a blank line, prose, an item continuation, a construct's last line) it carries the gap, first-of-list and interior markers alike. Spacing MUST NOT depend on the marker's role in a list: lazy continuation lets one character typed lines away merge or split lists — absorbing a blank or prose run into an item, or releasing it — flipping untouched markers between first-of-list and interior, so role-keyed spacing reflows lines the keystroke never touched, while the seam itself can change only by editing the marker line or the line directly above it (recorded divergence: GitHub renders a marker directly under a continuation line tight; the seam-local gap is the price of remote-edit invariance and matches the model's continuation-lines-read-as-paragraphs axiom) — interior blockquote/callout lines — quoted prose, quoted blank lines, and callout body lines at any depth except a callout's FIRST body line (the title→content seam stays the header's title padding, CALL-R-11), with in-quote lists following the same list rules via a probe that skips the lexical `>`/whitespace prefix (extends PARA-E-5's recorded divergence into quotes: `> a\n> b` renders joined on GitHub but gapped here) — AND the START line of every block construct: the opening fence of fenced code, the first line of indented code, of HTML/comment/processing-instruction blocks, of tables and block math, ATX heading lines, horizontal rules, and the FIRST line of the outermost blockquote or callout (its header), so every block separates from the content above it with the same rhythm prose uses. A construct-start gap MUST render as CLEAR space, never a tinted band: tinted line chrome bottom-anchors its background past the gap (fence header, indented-code first, HTML first, quote first `plainmark-blockquote-first`, callout header — quote bars start below the gap), constructs with their own breathing room STACK the gap above it (quote/callout `padding-y`, indented-code `padding-y`, HR padding — the gap resolves in the line's own em context) EXCEPT headings, which REPLACE their own padding-top with exactly the base-size gap — per-level rules divide it by the default size scale to cancel the heading's em context, so a heading sits one prose gap above the preceding block like every other line (`--plainmark-heading-padding-top` governs only non-gapped headings), and the block-WIDGET constructs (table, block math, and the image widget — a gap participant since line-scoped promotion, IMG-R-11) carry it as widget padding-top keyed on `plainmark-block-gap-above` when the block is not at the doc top. Not eligible: interior lines of all the constructs above (code body and closing fence, table rows past the first, HTML/math interiors), every frontmatter line, construct-start lines INSIDE a quote (quoted fences/tables/headings stay gap-free — the quote's outer gap belongs to its own first line; BQ-R-13), and marker lines at a TIGHT seam — a `ListItem` starts on the line AND another `ListItem` starts on the line directly above (a tight item run, nested and dedented marker lines) — where the adjacent-sibling item spacing owns the seam. Eligibility MUST be invariant across marker-typing transitions — `para\n-` (setext state), `para\n* ` (paragraph state), and `para\n* x` (list state) all keep the gap, so no marker keystroke moves the layout vertically; converting a continuation line to a bullet is a single hand-off, because `-` before existing text is still continuation prose and the space keystroke completes a real item in the same instant the gap yields to item spacing. Deepening an interior quote line with another `>` keeps the gap (the outermost quote still began earlier); converting a paragraph line into a fence, HR, heading, or quote first line KEEPS the gap through the transition (the construct start is itself eligible, so the keystroke causes no vertical jump); typing prose anywhere between two items — splitting the spanning list or merging blocks into it via lazy continuation — changes NO line's eligibility: blank lines are eligible in or out of a list, and marker spacing ignores the first-of-list vs interior role, so the only edits that move a marker line are on the marker line itself or on the line directly above it becoming or ceasing to be a marker line (a tight seam forming or dissolving — the moment a bullet is created or deleted there).
  _Example:_ `para\n- x\n- y` → the gap sits above `- x` (first list line); `- y` gets only the tight list-item spacing. `> a\n> b` → the gap sits above `b`, rendered as quote-tinted space with the nesting bar unbroken. `para\n> a` → clear gap above the quote box, then the quote's own tinted padding. `` para\n```js `` → clear gap above the fence header, code background starting below it; two stacked fenced blocks separate by one gap. `# a\n## b` → `## b` carries exactly one base-size gap (HEAD-R-9).
