...
- **MMD-E-11** `[divergent]` — Mermaid `mermaid.parse()` pre-validation is NOT used; the render path is `mermaid.render()` + `.catch` only. `suppressErrorRendering: true` makes invalid input reject with a message instead of rendering the bomb SVG, so the pre-gate is an unrealized optimization, not a correctness gap. Recorded as a conscious deviation, not a defect. _Correct/intended:_ a `mermaid.parse(src, { suppressErrors: true })` gate before `render`.
