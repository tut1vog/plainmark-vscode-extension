...
- **MMD-R-2** — A `FencedCode` block whose `CodeInfo` (trimmed, lowercased) equals `mermaid` and whose range does NOT overlap the main selection MUST be replaced by a `Decoration.replace({ block: true, widget: MermaidWidget })` spanning from the START of the opening fence's line through `node.to` (block decorations cover whole lines; an indented fence's node begins past the leading whitespace). The diagram source is the concatenation of ALL `CodeText` children sliced verbatim (empty string if none): inside a container the parser splits `CodeText` per line and strips the container indent, so the joined segments are the CommonMark block content, blank lines included.
...
- **MMD-E-6** — Each diagram source MUST be cached separately per theme: a result stored under one theme's key MUST NOT satisfy a widget rendered under another theme (`mermaid_cache_key(theme, src) = theme + ':' + src`). The theme key MUST name the color mode (`light`/`dark`) AND the resolved palette baked into the SVG (background, foreground, node fill, node border), so two themes of the same mode with different colors never share a render.
...
- **MMD-E-10** `[smoke]` — A theme switch MUST re-render every diagram, including a switch between two themes of the same color mode: each `mermaid_render_plugin` instance owns a `MutationObserver` on `document.body`'s `class` attribute and on the `<html>` element's `style` attribute (where VS Code writes the `--vscode-*` palette); when the recomputed theme key (MMD-E-6) differs it dispatches `set_mermaid_theme`, which updates `mermaid_theme_field`, rebuilds widgets under the new theme key, re-`initialize`s Mermaid with freshly resolved `themeVariables`, and re-renders. The observer is per-`ViewPlugin` instance (main view and any table-cell subview each own one) and disconnects in `destroy()`.
  _Example:_ switching VS Code from Dark+ to Light+ re-renders all visible diagrams with light-theme colors; switching from Dark+ to Monokai (both dark) re-renders them with Monokai's colors instead of keeping the Dark+ render.
...
- **MMD-E-12** `[smoke]` — A mermaid fence nested in a list item (indented to the item's content column) MUST render its diagram widget exactly like a top-level fence: the full multi-line source per MMD-R-2's concatenation (a first-segment-only source renders a bare `graph TD` header as an empty diagram — the "nothing rendered" failure) and the line-aligned replace so no stray indent fragment renders beside the widget. The widget box keeps its ordinary full-width chrome (MMD-R-8); the CBLK-R-17 content-column shift applies only to the caret-inside code chrome.
  _Example:_ `1. d:\n   ```mermaid\n   graph TD\n     A-->B\n   ``` ` off-caret → the A→B diagram renders; caret inside → shifted code chrome for editing.
...
- **MMD-E-13** — Every `mermaid.initialize` call (the bundle's boot and the per-theme re-initialize) MUST pass a `secure` list that extends Mermaid's defaults with `themeCSS`, so a `%%{init: {"themeCSS": …}}%%` directive in a diagram is dropped before render. Strict-mode DOMPurify keeps the SVG's `<style>` and the webview CSP allows inline styles, so unscoped directive CSS would otherwise apply to the whole editor.
  _Example:_ a fence starting `%%{init: {"themeCSS": ".cm-editor{display:none}"}}%%` renders its diagram and the editor stays visible.
