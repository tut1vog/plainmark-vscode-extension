...
- **LIST-R-3** `[smoke]` `[accepted]` — The bullet glyph MUST cycle by depth bucket — bucket 0 → filled disc, bucket 1 → hollow circle, bucket 2 (depth ≥ 2) → square — drawn by `.plainmark-list-bullet::before { content: var(--plainmark-list-bullet[-2|-3]) }` (defaults `●` / `○` / `■`), with the level-2/3 glyphs scaled down (`0.6em` / `0.5em`). The widget DOM is an empty `.plainmark-list-bullet` span; the glyph is pure CSS so a `plainmark.styles` override applies live. (Supersedes the single-`•`-all-depths plan — depth-cycling landed at T19.7.4; recorded for the matrix.)
  _Example:_ `- a\n  - b\n    - c` → glyphs `●`, `○`, `■` at the three depths.
...
- **LIST-E-4** `[smoke]` — Wrapped continuation lines and deeper nesting MUST align to the parent's text column via hanging-indent CSS (`.plainmark-list-item` `padding-left: calc((depth+1) * indent)` + negative `text-indent`, the bullet widget sized to exactly one `--plainmark-list-indent` unit). Indentation MUST use horizontal padding only (never vertical margin — T14.1 / T19.4b height-map desync). Consecutive sibling items MUST get `padding-top: var(--plainmark-list-item-spacing, 0.25em)` via `.plainmark-list-item + .plainmark-list-item`.
