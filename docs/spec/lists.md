...
- **LIST-R-2** `[smoke]` — When the item is not revealed AND the marker is followed by at least one character on its own line (the space-gate, see LIST-E-6), an unordered marker MUST be replaced by a `Decoration.replace` carrying a `ListBulletWidget`; the replaced span MUST cover the source's leading whitespace, the `ListMark` (`-`/`*`/`+`), AND the single trailing space (`[line.from, markEnd(+space)]`), so the fixed-width widget box alone forms the marker column. On a QUOTED line the span starts just past the quote prefix — the last `QuoteMark` plus its one trailing space — instead of `line.from`: the transparent `> ` run MUST stay in flow (its `QuoteMark` span draws the quote's nesting bar and backs the quote's hanging indent, BQ-R-11/BQ-R-12) while the nesting spaces after the prefix are still swallowed (LIST-R-11).
...
- **LIST-R-5** — An ordered marker (`1.` / `1)` …) MUST NOT be widget-replaced; when not revealed it is styled by a `plainmark-list-marker` `Decoration.mark` over `[mark.from, mark.to]`, and any leading whitespace before the mark is hidden by a `Decoration.replace({})`. On a QUOTED line the hide starts just past the quote prefix, leaving the `> ` run in flow (same boundary as LIST-R-2, see LIST-R-11). The digits are the rendered form (no CSS counter, no glyph).
  _Example:_ `1. a` → `1.` styled and shown verbatim; `  1. b` → leading two spaces hidden, `1.` styled.

- **LIST-R-6** `[smoke]` — When not revealed, a task item's `TaskMarker` (`[ ]` / `[x]` / `[X]`, exactly 3 bytes) MUST be replaced by a `Decoration.replace` carrying a `TaskCheckboxWidget` (`<input type="checkbox">`), and the leading `ListMark`-plus-space MUST be hidden by a zero-`font-size` `plainmark-list-marker-hidden` `Decoration.mark`. On a QUOTED line the hidden span starts just past the quote prefix, leaving the `> ` run in flow (same boundary as LIST-R-2, see LIST-R-11).
...
- **LIST-E-5** `[unknown]` — Caret/atomic-range and reveal behavior under a multi-line selection spanning several list items (mixed bullet / ordered / task) is not smoke-verified: ordered reveal fires per intersecting own-line while bullets/tasks stay widgeted. The composed GEOMETRY of a list inside a blockquote is now specified and smoke-verified (LIST-R-11, BQ-E-9), but the interaction of the list B2 no-reveal model with the quote's per-line reveal model remains unverified.
  _Example:_ select from `1. a` into `- b` → the ordered line is expected to reveal, the bullet line stays a widget — unconfirmed under F5.
