...
- **BQ-R-13** — Quote interiors MUST share the prose paragraph rhythm (PARA-R-7): every interior quote line at any depth — quoted prose, quoted blank lines (`>`), and quoted-list continuation lines — carries the paragraph gap, minus lines directly below an in-quote blank separator (PARA-R-13: the `>` line owns the seam's gap); interior gap padding sits inside the line box, so the quote background tints it and the per-marker bars (`top:0; bottom:0`, BQ-R-5) span it without a break. The quote's FIRST line ALSO carries the gap when the block sits directly below other content (not below a blank line — PARA-R-13) — rendered as CLEAR space above the block, never tint: the outermost quote's first line carries class `plainmark-blockquote-first`, stacks `--plainmark-paragraph-gap` + `--plainmark-blockquote-padding-y` as padding-top at (0,5,0), moves its tint from `background-color` to a bottom-anchored gradient sized `100% calc(100% − gap)`, and starts its bars at `top: gap`, so tint and bars meet the gap edge exactly. A doc-top quote takes no gap (padding-y only). In-quote lists follow the seam-local list rules (PARA-R-8), and quoted non-prose constructs (fences, tables, headings) stay gap-free.
