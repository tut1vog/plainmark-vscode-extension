...
- **AUTO-I-8** `[smoke]` — The full navigation bridge MUST carry the href to the host and open it: the webview document listener forwards `plainmark-link-click` as a `{ type: 'link_click', href }` host message and the host opens the resolved target (allowlisted schemes via `openExternal`; an off-allowlist scheme in an autolink is dropped like any other, SHELL-M-3). An empty href MUST be dropped at both bridge ends. Same bridge, scheme-allowlist, and degenerate-href handling as `links.md` LINK-I-9 / LINK-I-12.
