...
- **THEME-V-6** — The list indent/marker machinery MUST drive horizontal layout from custom properties: `--plainmark-list-depth` (set per line by the list-item line decoration), `--plainmark-list-indent`, and the per-level bullet glyphs `--plainmark-list-bullet`, `--plainmark-list-bullet-2`, `--plainmark-list-bullet-3`.
  _Example:_ overriding `--plainmark-list-bullet: "▸"` re-glyphs the top-level unordered bullet live without re-parsing source.
