...
- **CALL-R-11** — Callout bodies MUST share the prose paragraph rhythm (PARA-R-7), with one carve-out: the FIRST body line (directly under the header) carries no gap — the title→content seam stays `--plainmark-callout-title-padding-bottom` alone (detected via `detect_callout` on the outermost quote's header line). Later body lines carry the gap as padding inside the line box, so the accent gradient (CALL-R-5) spans it unbroken — minus lines directly below an in-quote blank separator (PARA-R-13: the `>` line owns the seam's gap). The HEADER line carries the block's OUTER gap when the callout sits directly below other content (not below a blank line — PARA-R-13): it stacks `--plainmark-paragraph-gap` + `--plainmark-callout-padding-y` as padding-top at (0,5,0), and BOTH background layers (accent bar and tint) bottom-anchor sized `calc(100% − gap)`, so the box separates with clear space and the accent never bleeds into the gap. A doc-top callout header takes no gap.
