...
- **CALL-R-11** — Callout bodies MUST share the prose paragraph rhythm (PARA-R-7), with one carve-out: the FIRST body line (directly under the header) carries no gap — the title→content seam stays `--plainmark-callout-title-padding-bottom` alone (owner smoke 2026-07-20 rejected a gap-sized seam under the icon line; detected via `detect_callout` on the outermost quote's header line). Later body lines carry the gap as padding inside the line box, so the accent gradient (CALL-R-5) spans it unbroken. The HEADER line carries the block's OUTER gap when the callout sits below other content: it stacks `--plainmark-paragraph-gap` + `--plainmark-callout-padding-y` as padding-top at (0,5,0), and BOTH background layers (accent bar and tint) bottom-anchor sized `calc(100% − gap)`, so the box separates with clear space and the accent never bleeds into the gap. A doc-top callout header takes no gap.
