...
- **CALL-R-11** — Callout bodies MUST share the prose paragraph rhythm (PARA-R-7), with one carve-out: the FIRST body line (directly under the header) carries no gap — the title→content seam stays `--plainmark-callout-title-padding-bottom` alone (detected via `detect_callout` on the outermost quote's header line). Later body lines carry the gap as padding inside the line box, so the accent gradient (CALL-R-5) spans it unbroken. The HEADER line carries the block's OUTER gap when the callout sits below other content: it stacks `--plainmark-paragraph-gap` + `--plainmark-callout-padding-y` as padding-top at (0,5,0), and BOTH background layers (accent bar and tint) bottom-anchor sized `calc(100% − gap)`, so the box separates with clear space and the accent never bleeds into the gap. A doc-top callout header takes no gap.
  _Example:_ `> [!note] t\n> body\n> more` → the gap sits above `more` only; `body` hugs the title at the title-padding seam. `para\n> [!note] t` → clear gap between `para` and the callout box, the accent bar starting below it.

- **CALL-R-12** `[smoke]` — A callout nested in list items (≥1 enclosing `ListItem`, outside any `Blockquote`) MUST take the BQ-R-14 list-nested geometry: every header and body line carries class `plainmark-callout-nested` plus `--plainmark-quote-nest: <n>`, the theme insets the line by a transparent `border-left` of `n × --plainmark-list-indent` with `background-clip: padding-box`, and — because both CALL-R-5 gradient layers are positioned in the padding box — the accent bar and tint land on the list content column with no per-type rule change. The CALL-R-10 inline hanging indent stays net-to-zero inside the border and its prefix scan starts at the line's first `>`, so the bar-to-title and bar-to-body offsets equal a top-level callout's. The whitespace run before each line's first `>` is hidden by the same widgetless `Decoration.replace({})` (tab-proof, BQ-R-14), in both caret states (a top-level `  > [!NOTE]` renders flush). Inside another `Blockquote` the geometry MUST NOT apply (BQ-E-14).
  _Example:_ `- item\n  > [!NOTE] Title\n  > body` → the accent bar starts under `item`; title and body sit the same distance from the bar as in a top-level callout.
...
- **CALL-E-8** `[unknown]` — A callout composes inside a list (geometry per CALL-R-12) or other container, and inline constructs (footnotes, links, emphasis) compose inside callout body lines, but the interaction of the callout reveal model with a surrounding/contained construct's own reveal model is not verified and may surface as a bug. A callout marker on a quote that opens on the item's own marker line (`- > [!NOTE]`) is not detected — the CALL-R-1 prefix strip requires the line to start with the quote prefix — so it renders as a plain nested quote (BQ-R-14) with the literal `[!NOTE]` visible; and a list inside a callout takes no depth step (the LIST-R-11 marker step is scoped to plain quotes). Both are unverified compositions.
  _Example:_ `- item\n  > [!NOTE]\n  > body` → callout chrome at the list content column (CALL-R-12); `- > [!NOTE] t` → plain nested quote reading `[!NOTE] t`.
