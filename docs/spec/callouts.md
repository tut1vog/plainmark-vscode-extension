...
- **CALL-E-8** `[unknown]` — A callout composes inside a list (geometry per CALL-R-12) or other container, and inline constructs (footnotes, links, emphasis) compose inside callout body lines, but the interaction of the callout reveal model with a surrounding/contained construct's own reveal model is not verified and may surface as a bug. A callout opening on the item's own marker line (`- > [!NOTE]`) keeps the list marker in flow ahead of the accent — the LIST-R-11 marker pull-back is scoped to plain quotes — and a list inside a callout takes no depth step; both are unverified compositions.
  _Example:_ `- item\n  > [!NOTE]\n  > body` → callout chrome at the list content column (CALL-R-12); `- > [!NOTE]` → renders, the marker/accent overlap unverified.
