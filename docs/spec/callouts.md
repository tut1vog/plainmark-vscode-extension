...
- **CALL-R-11** — Callout bodies MUST share the prose paragraph rhythm (PARA-R-7, ADR-0007), with one carve-out: the FIRST body line (directly under the header) carries no gap — the title→content seam stays `--plainmark-callout-title-padding-bottom` alone (owner smoke 2026-07-20 rejected a gap-sized seam under the icon line; detected via `detect_callout` on the outermost quote's header line). Later body lines carry the gap; the header line carries none (its spacing stays CALL-R-4's chrome). The gap is padding inside the line box, so the accent gradient (CALL-R-5) spans it unbroken.
  _Example:_ `> [!note] t\n> body\n> more` → the gap sits above `more` only; `body` hugs the title at the title-padding seam.
