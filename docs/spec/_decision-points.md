...
| CALL-E-5 | Obsidian pipe-metadata syntax (`[!NOTE\|meta]`) is not parsed; the `\|` breaks the type match so the line renders as a plain blockquote. Pipe-metadata deferred. | docs/spec/callouts.md | pending |
...
| MATH-E-3 | Permissive Typora-style inline-math matching; currency pairs typeset as math, mitigated by `\$` escapes. Retagged from `[divergent]` — the permissiveness is the chosen design, pinned by `math.test.ts`. | docs/spec/math.md | blessed |
| MMD-E-11 | No `mermaid.parse()` pre-gate; render+catch with `suppressErrorRendering` is the accepted path. Retagged from `[divergent]` — no user-visible difference. | docs/spec/mermaid.md | blessed |
| TBL-I-19 | Paste conversion was deferred wholesale; document-level HTML-`<table>`/TSV conversion now ships (TBL-I-35..37, owner-approved), so that part of the deferral is reopened. | docs/spec/tables.md | reopened |
| TBL-I-19 | Cell-targeted paste stays literal: multi-cell distribute paste (PD–PG) and CSV detection remain deferred; a paste into an active cell inserts plain text. | docs/spec/tables.md | pending |
| PASTE-H-7 | Rich-text paste maps semantic tags only: style-mapped bold/italic (`font-weight: 700` / `font-style: italic` spans — Google Docs, Notion) and unmapped inline tags (`u`, `sup`, `kbd`, …) degrade to plain text; Ctrl+Shift+V on Desktop is unverified as a plain-text escape (the menu Paste is the documented one). | docs/spec/paste.md | pending |
| TBL-E-7 | A pipe-bearing line directly under a table is absorbed as a table row (GFM continuation semantics) and the first edit MC1-normalizes it, dropping cells past the header column count — silent data loss accepted rather than fighting the parser. A blank line is the user's separator. | docs/spec/tables.md | blessed |
| TBL-E-15 | A delimiter row with trailing spaces (`|:--|--:|␠␠`) is not parsed as a table by `@lezer/markdown`, while GitHub accepts it; upstream divergence left as-is rather than trimming source. | docs/spec/tables.md | pending |
