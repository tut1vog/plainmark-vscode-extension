...
| MATH-E-3 | Permissive Typora-style inline-math matching; currency pairs typeset as math, mitigated by `\$` escapes. Retagged from `[divergent]` by ADR-0003 — the permissiveness is the chosen design, pinned by `math.test.ts`. | ADR-0003 | blessed |
| MMD-E-11 | No `mermaid.parse()` pre-gate; render+catch with `suppressErrorRendering` is the accepted path. Retagged from `[divergent]` by ADR-0003 — no user-visible difference. | ADR-0003 | blessed |
