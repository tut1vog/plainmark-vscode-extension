...
- **IMG-I-1** — When the canonical reveal predicate (`should_reveal_for_selection`, MRS-R-2…R-5: any selection range touching the image's LINE range reveals, EXCEPT a non-empty selection strictly covering it on both sides; pointer-down evaluates the frozen pre-press selection) holds for the image line's range, the replace widget MUST NOT be emitted, revealing the raw `![alt](url)` source for editing — with an in-flow preview below it (IMG-I-11). Reveal is keyed to the image's line, so a caret on a sibling line of the same merged paragraph keeps the widget rendered. Under the unified reveal model: select-all keeps the image rendered; a drag entering the image does not flash raw source mid-drag.
...
- **IMG-I-6** `[smoke]` — When one or more image blobs are present on the clipboard at paste time, the paste handler MUST suppress the default paste, read each blob's bytes and MIME type, and request a host-side save; on a successful save it MUST insert `![](relative-path)` with empty alt text at the caret, the path percent-encoded so it is a valid bare CommonMark destination (spaces, parentheses, `#`, `?`, `%` and friends escaped; non-ASCII letters kept). The inserted path MUST be the saved file's location expressed relative to the document's folder, so it resolves through the image base (IMG-I-3, IMG-R-7). When no image blob is present (plain text or a URL only), paste MUST fall through to the rest of the paste chain (table conversion when the payload qualifies, TBL-I-35; rich-text conversion when the HTML carries formatting, PASTE-H-1; otherwise the default plain-text insert) — no remote image is downloaded or localized.
...
- **IMG-I-8** `[smoke]` — When the document has no writable folder to save beside — an untitled buffer (no document folder), or any document scheme reporting `isWritableFileSystem` false — the host MUST NOT attempt a write; it MUST reply with an error and surface an actionable message ("Save this document to a folder before pasting images"). A saved file outside any open workspace folder is NOT refused: the image saves relative to the document's own folder (IMG-I-7). A data-URI fallback is explicitly deferred as a possible later opt-in.
  _Example:_ pasting an image into an unsaved (untitled) buffer inserts nothing and shows the "save this document to a folder" warning; pasting into a saved document opened with no folder saves the image beside that document.
