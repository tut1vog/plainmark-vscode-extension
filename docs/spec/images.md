...
- **IMG-R-11** `[smoke]` — An image widget replacing a line directly below content (any line other than doc line 1 or a line directly below a blank line — PARA-R-13) MUST carry `plainmark-block-gap-above`, taking the paragraph gap (`var(--plainmark-paragraph-gap, 0.75em)`) as widget padding-top — the image container has no breathing of its own, so the gap is the whole padding (PARA-R-7; mirrors the table and block-math widgets). A doc-top or below-blank image takes none. The in-flow preview (IMG-I-11) never takes it — the revealed source line above it carries its own gap. `gap_above` participates in `ImageWidget.eq`, so an edit moving the image across the doc-top or blank-seam boundary redraws the widget.
