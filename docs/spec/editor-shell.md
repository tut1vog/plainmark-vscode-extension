...
- **SHELL-X-16** `[smoke]` — In-document find MUST be composed into `editor_extensions` (main view only — NOT `editor_extensions_core`, so table cell subviews share no panel): `@codemirror/search`'s `search({ top: true, createPanel })` with Plainmark's own panel, a find-only slice of `searchKeymap` (the `Mod-d` / `Mod-Alt-g` / `Mod-Shift-l` multi-cursor / go-to-line bindings removed) at `Prec.high`, and a panel theme. The panel MUST be a floating widget anchored to the editor's top-right corner — absolutely positioned over the scroller, never an in-flow row that pushes the document down — with find / replace fields, previous / next / replace / replace-all buttons, match-case / regex / whole-word options, and a close button; its controls MUST take the workbench font (`--vscode-font-family` / `--vscode-font-size`), not CM6's 70% baseTheme sizing. CM6 search scans the `EditorState` document model, so a match on an off-screen (virtualized) line, a `display:none` marker, or inside a replaced widget (table / math) is still found — a rendered-DOM text search would miss it. `Prec.high` is load-bearing: `Mod-f` resolves to `Ctrl-f` on Windows/Linux, so the find keymap MUST pre-empt `defaultKeymap`'s `Ctrl-f` (`cursorCharRight`) and `Escape` (`simplifySelection`); the panel's own keydown handler MUST route through the `search-panel` scope so Escape / F3 / Mod-g work from inside its fields. The panel and match colors MUST route through `--vscode-*` vars (CM6's panel baseTheme is light-only). A match inside inline code MUST stay visible — the code mark nests inside the lower-precedence search mark and would otherwise paint its background over it. A `cm-searchMatch` highlight MAY be occluded where a replace-widget (math / mermaid) covers the matched bytes until it becomes the current match, whose selection reveals the source. Matches inside table cells are painted per `TBL-R-19`. The host-side Ctrl/Cmd+F muzzle is `SHELL-C-12`.
  _Example:_ Ctrl/Cmd+F opens a compact find widget in the top-right corner; typing a word highlights every occurrence document-wide and Enter / F3 walk them, including matches on lines scrolled out of view.
- **SHELL-X-17** — The find field MUST apply its text only after a short typing pause (200 ms without input), so a half-typed word does not light up every letter in the document; Enter / Shift+Enter, the panel buttons, and the `search-panel` scope keys (F3 / Mod-g / Escape) MUST flush any pending text first so they act on what the field shows. Option toggles and the replace field apply immediately. The current match (`cm-searchMatch-selected`) MUST read apart from the other matches: `--vscode-editor-findMatchBackground` plus a 1px outline chaining `--vscode-editor-findMatchBorder` → `--vscode-focusBorder`, which stays visible through the translucent selection wash; other matches take `--vscode-editor-findMatchHighlightBackground` with a `--vscode-editor-findMatchHighlightBorder` outline.
  _Example:_ Typing `al` then `pha` in one burst highlights nothing until the typing pauses, then every `alpha`; Enter jumps to the first one, which gains a focus-colored outline while the rest stay in the highlight color.
- **SHELL-X-18** — The find panel MUST show a match count beside the find field, recomputed from the document model on every query, document, or selection change: `N matches` (`1 match`) while no match is selected, `k of N` once the selection sits on a match, `No results` (in the error color) for a valid query with no match, and nothing for an empty query. The scan is capped at 1000 matches, past which the total reads `1000+`.
  _Example:_ searching `the` in a long note shows `42 matches`; Enter turns it into `1 of 42`, F3 into `2 of 42`; searching `zzz` shows `No results` in red.
- **SHELL-X-19** — When a new search text (or a changed option) applies, the panel MUST select an initial match: the first match at or below the top of the on-screen area (so a match already on screen wins, else the first one below it), wrapping to the first match in the document when nothing lies at or below the screen; the selection is scrolled into view without moving focus out of the panel. Enter / Shift+Enter and the step buttons that themselves flush pending text MUST stay on that initial match rather than stepping past it; the replace field's text changes select nothing.
  _Example:_ with lines 5, 30, and 55 matching and the view scrolled to line 10, typing the query selects line 30's match (`1 of 3`); scrolled past line 55 it selects line 5's; Enter typed straight after the word lands on that same match, and the next Enter steps to the following one.
...
- **SHELL-M-3** — A `link_click` whose RFC-3986 scheme is on the external allowlist (`http`, `https`, `mailto`, `vscode`, `vscode-insiders`; matched case-insensitively) MUST be opened with `vscode.env.openExternal`; a `file:` href MUST be opened in-editor with `vscode.open`, never `openExternal`; any other scheme-bearing href MUST be dropped with no side effects; a document-relative href MUST be percent-decoded (falling back to the raw string on a malformed escape), resolved against the document directory, and opened with `vscode.open`; a bare `#fragment` MUST be ignored; a relative href on a parentless (e.g. `untitled:`) document MUST be dropped.
  _Example:_ `link_click` `https://x` → `openExternal`; `file:///notes.md` → `vscode.open`; `javascript:alert(1)` → dropped; `./img.png` → `vscode.open(joinPath(docDir,'./img.png'))`; `a%20b.pdf` → `vscode.open(joinPath(docDir,'a b.pdf'))`; `#sec` → ignored.
...
- **SHELL-C-13** `[smoke]` — The provider MUST maintain a single right-aligned status bar item showing the active Plainmark document's word count over the raw source: each Han, Hiragana, or Katakana character counts as one word (unspaced scripts), CJK punctuation acts as a separator without counting, and all remaining text counts as whitespace-separated non-whitespace runs (markdown markers count as words), labeled `N Words` with a `1 Word` singular. It MUST refresh on panel resolve, view-state change, document change, and panel disposal, and MUST be hidden whenever no Plainmark panel is active. The document-change refresh MUST be debounced (one rescan shortly after a burst of change events, not one per event — each keystroke fires two or more, once per bound panel) and the count MUST be cached by document version so view-state changes reuse it — a custom-editor tab surfaces none of the built-in text-editor status entries (Ln/Col, selection), so this item is the tab's only document indicator.
...
- **SHELL-C-17** `[smoke]` — `tutivog.plainmark.prettifyDocument` MUST be registered and, when invoked, MUST post `{ type: 'prettify_seams', seams }` to the active Plainmark panel's webview, where `seams` is `plainmark.prettify.seams` resolved at invocation time against the panel's document URI (PARA-I-8) and every resolution warning is logged. Reading the setting per invocation rather than injecting it means a settings edit takes effect on the next run with no view reload. The document transform it triggers is owned by `PARA-I-7`.
  _Example:_ Command Palette → "Plainmark: Prettify document (normalize blank lines between blocks)" → active panel receives `prettify_seams` carrying the resolved override list.
