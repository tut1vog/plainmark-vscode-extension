...
- **TBL-SP-4** — **(E1) Over-escape all pipes and backslashes.** On serialize, `escape_cell_text` MUST rewrite every `\` in cell text to `\\` and every `|` to `\|`, in that order, with NO code-span exception (technically over-escape per GFM, benign). Backslash escaping is required, not cosmetic: GFM consumes a backslash before any ASCII punctuation, so a cell whose logical text places a `\` immediately before a `|` would otherwise serialize to `\\|` — read back as an escaped backslash plus a live delimiter pipe, splitting the cell. On parse, `parse_cell_text` MUST reverse both in one left-to-right pass (`\\`→`\`, `\|`→`|`).
  _Example:_ a cell containing `a|b` serializes as `a\|b`; a cell whose logical text is `a\|b` (literal backslash then literal pipe) serializes as `a\\\|b`; both reopen to their original logical text.
