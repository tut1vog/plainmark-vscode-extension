...
- **MATH-E-9** `[accepted]` — The TeX surface is limited to the bundled packages (TeX-base + AMS + boldsymbol + newcommand); `autoload` and `\require` are omitted, so packages outside that set are unavailable. Macros are rendered with the newcm font and all dynamic glyph ranges preloaded (no runtime CDN fetch, no `□` boxes).
