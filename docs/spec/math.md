...
- **MATH-R-2** — An off-caret `InlineMath` node MUST be replaced by an inline (non-block) `Decoration.replace` widget over its `[from, to)` range. An off-caret `BlockMath` node MUST be replaced by a `block: true` `Decoration.replace` widget covering **whole lines**: the node's `[from, to)` extended to its first line's start and last line's end. That extension is legal only over pure-whitespace margins (MATH-E-5 leading indent / trailing spaces); when any non-whitespace shares a line with the node (quote `> ` or list `- ` prefixes), no legal range exists and the field MUST emit no block replace widget for that node (MATH-E-13).
  _Example:_ `value: $x = y$` (caret outside) → one inline replace widget over `$x = y$`; `$$\na = b\n$$` (caret outside) → one block replace widget over the whole block; `  $$x$$   ` → one block replace widget over the entire line including the indent and trailing spaces; `> $$x$$` → no block replace widget.
...
- **MATH-E-13** — Display math (`$$…$$`) nested inside a blockquote, callout, or list item is NOT rendered as a typeset block; the raw `$$…$$` source stays visible and the widget field MUST emit no replace widget for the node (MATH-R-2). The grammar emits a `BlockMath` node that begins after the line's `> ` / `- ` prefix (and, in the multi-line quoted form, straddles the inner `QuoteMark` tokens), so the node has a partial-line range with non-whitespace outside it — no whole-line replace range exists. A partial-line `block: true` replace over such a node is undefined behavior in CM6: it splits the opener line into a `> ` stub with the widget outside the quote chrome, and mis-maps DOM-side edits near the widget into document edits — one DOM-path Backspace on the line below deletes the entire block, and a DOM selection reaching into the widget's MathJax text (v4 CHTML carries the formula's unicode rendering as real DOM text) injects or removes source bytes — INV-SP-1 violations. Inline math (`$…$`) inside a blockquote is unaffected: it parses as a single-line `InlineMath` whose source extraction (MATH-SP-2) carries no quote marker, and it renders normally (cross-ref `blockquotes.md` BQ-E-9).
  _Example:_ `> $$a = b$$`, `> $$\n> a = b\n> $$`, `- $$a = b$$`, or a `> [!NOTE]` body block → no typeset display block and no replace widget; the raw source stays visible and every byte is edit-safe. `> $x$` → inline math renders as usual.
