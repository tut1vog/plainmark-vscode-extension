...
- **MATH-R-2** — An off-caret `InlineMath` node MUST be replaced by an inline (non-block) `Decoration.replace` widget over its `[from, to)` range; an off-caret `BlockMath` node MUST be replaced by a `block: true` `Decoration.replace` widget over its `[from, to)` range. The replaced range covers the `$`/`$$` delimiters and the LaTeX between them.
  _Example:_ `value: $x = y$` (caret outside) → one inline replace widget over `$x = y$`; `$$\na = b\n$$` (caret outside) → one block replace widget over the whole block.
...
- **MATH-R-7** `[smoke]` — The block widget MUST be styled `text-align: var(--plainmark-math-align, center)`, `font-size: var(--plainmark-math-size, 1.21em)`, `color: var(--plainmark-math-color, inherit)`, `padding: var(--plainmark-math-padding, 0.25em 0)`, `margin: 0`; the inline widget inherits color from the same `--plainmark-math-color`.
  _Example:_ `$$E=mc^2$$` renders centered at ~1.21em; a theme may left-align it via `--plainmark-math-align`.
...
- **MATH-I-2** — A `BlockMath` node whose range overlaps the selection MUST NOT render its replace widget; the raw `$$…$$` source (all lines) is exposed for editing.
  _Example:_ `$$\na = |b\n$$` → all three source lines shown and editable.
...
- **MATH-I-4** `[smoke]` `[unknown]` — The replace widget overrides `ignoreEvent()` to return `false` (CM6's default `true` swallows clicks), so a click on the rendered math places the caret and triggers the MATH-I-1/I-2 source reveal. Confirming the live click-to-edit behavior requires a smoke check.
  _Example:_ clicking the rendered `$x^2$` places the caret inside the span and reveals `$x^2$`.
...
- **MATH-SP-2** `[inherits:INV-SP-1]` — The LaTeX source slices used for typesetting MUST be read-only: `find_inline_math_source` strips one `$` from each end and `find_block_math_source` strips the `$$` markers, but neither writes back; the stripped value feeds only MathJax and the cache key.
  _Example:_ the `$$` markers of `$$a=b$$` never disappear from the document; only the typeset *view* omits them.
...
- **MATH-E-3** `[divergent]` — Inline math matches **permissively** (Typora-style): any two unescaped `$` on one line delimit math regardless of surrounding text, so an interior-space-bearing currency pair typesets as math. Correct behavior (deterministic-glyph / currency-safety intent) would not treat adjacent dollar amounts as math; today the chosen mitigation is that users escape literal dollars with `\$`. (`\$` is consumed by lezer-markdown's built-in `Escape` parser before the math rule sees it, so `\$5` is safe; the backslash itself is hidden in live preview and the dollar renders as a literal — see `escapes.md` ESC-R-1 / ESC-E-5.)
...
- **MATH-E-13** `[accepted]` — Display math (`$$…$$`) nested inside a blockquote or callout is NOT rendered as a typeset block. The grammar still emits a `BlockMath` node spanning the quoted lines, but the node range straddles the inner `>` `QuoteMark` tokens, which the render layer does not account for: `find_block_math_source` slices the raw range and strips only the `$$` markers, so the LaTeX handed to MathJax still contains the literal `> ` quote prefixes, and the `block: true` replace widget begins mid-line (after the opener's `> `), conflicting with the blockquote's per-line chrome (BQ-R-2/BQ-R-11). The result is mis-typeset or unrendered math, so block-math-in-blockquote is deferred (Obsidian parity — it likewise does not render block math inside a blockquote). Inline math (`$…$`) inside a blockquote is unaffected: it parses as a single-line `InlineMath` whose source extraction (MATH-SP-2) carries no quote marker, and it renders normally (cross-ref `blockquotes.md` BQ-E-9).
  _Example:_ `> $$\na = b\n> $$` (or `> [!NOTE]` body) → no typeset display block; the raw `$$…$$` source stays visible. `> $x$` → inline math renders as usual.
