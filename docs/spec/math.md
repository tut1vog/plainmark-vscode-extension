...
- **MATH-I-2** — A `BlockMath` node whose **whole-line widget span** (`block_math_widget_range`, MATH-R-2) overlaps the selection MUST NOT render its replace widget; the raw `$$…$$` source (all lines) is exposed for editing. Keying the overlap on the widget span (not the bare node range) makes a caret on the replaced line's margins — the indent of `  $$x$$`, the `> ` prefix of a quoted block — reveal the source, matching what the widget visually covers.
  _Example:_ `$$\na = |b\n$$` → all three source lines shown and editable; `> |$$x$$` (caret on the quote prefix) → source revealed.
