...
The two paragraph commands convert a document between blank-line-separated markdown (the conventional style most tools emit) and Plainmark's compact one-line-per-paragraph style. **Compact paragraphs** joins hard-wrapped lines (without inserting spaces between CJK characters) and removes the blank lines between paragraphs, headings, and lists; **Convert to CommonMark** does the reverse, producing conventional spacing so every paragraph break survives wherever the file is rendered — including inside blockquotes and callouts, where it writes the quoted blank line (a `>` on its own line) between paragraphs. Both apply as a single undoable edit and skip anything a rewrite could change the meaning of — hard line breaks, code blocks, tables, and list interiors.
